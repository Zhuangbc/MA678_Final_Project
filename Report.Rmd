...
pacman::p_load("tidyverse","gridExtra","arm","rstanarm","magrittr","kableExtra")
```
...
2. For many cities, they have very few restaurants. The number of restaurants in each city is shown in *Appendix: Figure*. It would be difficult to model using classical regression. Multilevel model can give us reasonable estimates even for cities with small sample sizes.

We first tried multilevel linear model. Our predictors are city ID, flavor rating, environment rating and service rating. However, the parallel lines in residual plot indicated that treating total score as a continuous variable is not suitable. Results are shown in *Appendix: Figure*. Therefore, we decided to treat total score as an ordered categorical variable and used multilevel ordinal logistic regression model. We ran Bayesian model using *brms* in R to build this model. We used *nominal test* to check the proportional odds assumption. We assessed the model using Bayesian posterior predictive checking. 
...
We allowed all variables to vary between cities. In *Figure*, we can see the estimates $\pm$2 standard errors for intercepts, flavor rating, environment rating and service rating. 
```{r,echo=FALSE,message=FALSE, warning=FALSE,results="asis",fig.cap="\\label{fig:figs} Density estimates of the total score data y and replicates",fig.width=10}
pp_check(fit5)
```

## 4.2 Predictions
We assess the model by Bayesian posterior predictive checking. *Figure* shows the overlaid density estimates of the data and the replicates. If our model fits the data well, we would expect that the data and the replicates will overlap perfectly. From the result, we can see that they almost overlap perfectly. Our model captures the patterns very well. Though, we still lose some features in those peaks. The overall prediction performance is great.
```{r,echo=FALSE,message=FALSE, warning=FALSE,results="asis",fig.cap="\\label{fig:figs} Density estimates of the total score data y and replicates",fig.width=10}
pp_check(fit5)
```
...
Then, we look at the city level estimations. Estimations in city level tell us that this rating effects vary between cities. We look at the cities that flavor rating has a relatively large effect on the total score. In *Figure*, cities are sorted by the coefficient of flavor rating. The first three cities are . In China, these cities are all.
...
# 7 Appendix
```{r,echo=FALSE,message=FALSE, warning=FALSE,results="asis",fig.cap="\\label{fig:figs} Distribution of overall rating, flavor rating, environment rating and service rating before sampling"}
#Score
p_score <- ggplot(rest)+
...
```{r,echo=FALSE,message=FALSE, warning=FALSE,results="asis",fig.cap="\\label{fig:figs} Distribution of overall rating, flavor rating, environment rating and service rating after sampling"}
#Score
...
```{r,echo=FALSE,message=FALSE, warning=FALSE,results="asis",fig.cap="\\label{fig:figs} The number of restaurants in each city"}
...
```{r,echo=FALSE,message=FALSE, warning=FALSE,results="asis",fig.cap="\\label{fig:figs} Residual plot and Q-Q plot of linear mixed effects model"}
#Model
rest_sub <- rest_sub %>%
